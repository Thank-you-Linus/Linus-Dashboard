"""
Group Entity Manager for Linus Dashboard.
...
Ported from Linus Brain (utils/group_manager.py), which already solved the
cold-restart race condition this pattern exists to avoid.

Used by:
- binary_sensor.py (presence + per-device_class groups)
- light.py, switch.py, fan.py, cover.py, siren.py (domain groups)
"""
        ...
        try:
            ...
            if self._check_empty_callback and self._removal_callback:
                if self._check_empty_callback():
                    _LOGGER.info(
                        "%s: Group is empty, triggering removal", self._log_prefix
                    )
        ...
        if not self._startup_complete:
            ...
            unsubs.append(
                self.hass.bus.async_listen_once(
                    EVENT_HOMEASSISTANT_STARTED, _ha_started
                )
            )
        else:
